...
async function closeAllCandidatures(req, res, next) {
  ...
  try {
    ...
    if (current[0].PK_company !== req.auth.id1 && req.auth.role !== "admin") {
      throw generateError(
    ...
    const [
      ...
      "UPDATE candidatures ca inner join projects po on ca.FK_candidature_project=po.PK_project inner join companies co on po.FK_project_company=co.PK_company SET ca.candidature_finished=1,ca.modification_date=? WHERE po.FK_project_company=? and ca.FK_candidature_project=?",
      [date, id1, id2]
    );
